...
const CATEGORY_SLUG = "whey";
const CATEGORY_NAME = "Whey Protein";
...
const categoryDisplayConfig = {
  settings: {
    ...
    customSorts: [
      {
        value: "protein_dose",
        label: "Mais proteina por dose",
        attributeKey: "proteinPerDoseInGrams",
        direction: "desc",
      },
      {
        value: "protein_percent",
        label: "Maior % de proteina",
        attributeKey: "proteinPercentage",
        direction: "desc",
      },
      {
        value: "dose_price",
...
function buildAttributes(
  ...
  if (!info) {
    return {
      attributes: {
        ...baseAttributes,
        migrationPendingReason: "Faltou wheyInfo no legado",
      },
      issue: "faltou wheyInfo no legado",
    };
  }

  const proteinPercentage =
    info.doseInGrams > 0 && info.proteinPerDoseInGrams > 0
      ? Number(((info.proteinPerDoseInGrams / info.doseInGrams) * 100).toFixed(2))
      : 0;
  ...
  const totalProteinInGrams =
    numberOfDoses > 0 && info.proteinPerDoseInGrams > 0
      ? Number((numberOfDoses * info.proteinPerDoseInGrams).toFixed(2))
      : 0;
...
async function main() {
  ...
  console.log(
    `Iniciando migracao de ${legacyProducts.length} produtos de whey...`
  );
...
main()
  .catch((error) => {
    console.error("Erro na migracao de whey:", error);
    process.exit(1);
  })
  .finally(async () => {
