...
async function fetchAmazonRatings(asin: string) {
  ...
  try {
    ...
    // 🔍 Seletores robustos para Rating (tenta o ID primeiro, depois a classe)
...
async function main() {
  ...
  // 1. BUSCA INTELIGENTE (Prioriza Lote Novo e Vencidos)
  const products = await prisma.dynamicProduct.findMany({
    where: {
      OR: [
        { ratingsUpdatedAt: null }, // Lote novo (Papel Higiênico, etc)
        { ratingsUpdatedAt: { lt: trintaDiasAtras } } // Vencidos há + de 1 mês
      ]
    },
    select: { id: true, asin: true, name: true },
    take: 100 // Processa 100 por rodada para não ser bloqueado
  });
  ...
  for (const product of products) {
    console.log(`🔍 [${product.asin}] - Processando: ${product.name.substring(0, 45)}...`);
    
    const result = await fetchAmazonRatings(product.asin);
    
    // Atualiza o banco mesmo se der erro (para carimbar a data e tirar da fila de "novos")
    // Se der erro 404, marcamos como atualizado para não tentar de novo amanhã
    await prisma.dynamicProduct.update({
      where: { id: product.id },
      data: {
        ratingAverage: result?.rating ?? undefined,
        ratingCount: result?.count ?? undefined,
        ratingsUpdatedAt: new Date() // 🔥 Carimba a data para sair da fila por 30 dias
      }
    });
    ...
    // Delay de 5 segundos (Segurança extra contra bloqueio de IP)
