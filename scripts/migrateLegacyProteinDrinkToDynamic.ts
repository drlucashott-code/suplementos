...
const CATEGORY_SLUG = "bebidaproteica";
const CATEGORY_NAME = "Bebida Proteica";
const CATEGORY_IMAGE =
  "https://m.media-amazon.com/images/I/51npzHic1NL._AC_SL1000_.jpg";

const categoryDisplayConfig = {
  settings: {
    analysisTitleTemplate: "ANALISE POR UNIDADE ({volumePerUnitInMl}ML)",
    ...
    customSorts: [
      {
        ...
        attributeKey: "proteinPerUnitInGrams",
        direction: "desc",
      },
      {
        value: "cheapest_unit",
        label: "Menor preco por unidade",
        attributeKey: "precoPorUnidade",
        direction: "asc",
      },
    ],
  },
...
function buildAttributes(
  ...
  if (!info) {
    return {
      attributes: {
        ...baseAttributes,
        migrationPendingReason: "Faltou proteinDrinkInfo no legado",
      },
      issue: "faltou proteinDrinkInfo no legado",
    };
  }

  const totalProteinInGrams =
    info.unitsPerPack > 0 && info.proteinPerUnitInGrams > 0
      ? Number((info.unitsPerPack * info.proteinPerUnitInGrams).toFixed(2))
      : 0;

  return {
    attributes: {
      ...baseAttributes,
      unitsPerPack: info.unitsPerPack,
      volumePerUnitInMl: info.volumePerUnitInMl,
      proteinPerUnitInGrams: info.proteinPerUnitInGrams,
      totalProteinInGrams,
    },
    issue: null,
  };
...
async function main() {
  ...
  console.log(
    `Iniciando migracao de ${legacyProducts.length} produtos de bebida proteica...`
  );
...
main()
  .catch((error) => {
    console.error("Erro na migracao de bebida proteica:", error);
    process.exit(1);
  })
  .finally(async () => {
