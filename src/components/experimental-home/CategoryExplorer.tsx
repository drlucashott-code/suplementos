...
const hubDetails: Record<HomeHub, { label: string; icon: React.ReactNode }> = {
  ...
  casa: { label: "Casa", icon: <Home className="h-4 w-4" /> },
...
function orderCategories(hub: HomeHub, categories: ExperimentalCategory[]) {
  if (hub !== "suplementos") return categories;

  const bySlug = new Map(categories.map((category) => [category.path.split("/").at(-1), category]));
  return supplementOrder.flatMap((slug) => {
...
export function CategoryExplorer({ categories }: { categories: Record<HomeHub, ExperimentalCategory[]> }) {
  const availableHubs = (Object.keys(hubDetails) as HomeHub[]).filter((hub) => categories[hub].length > 0);
  const [activeHub, setActiveHub] = useState<HomeHub>(availableHubs[0] || "suplementos");
  const [activePage, setActivePage] = useState(0);
  const railRef = useRef<HTMLDivElement>(null);
  const activeCategories = orderCategories(activeHub, categories[activeHub]);
  const categoryColumns = Array.from(
    { length: Math.ceil(activeCategories.length / 2) },
    (_, index) => activeCategories.slice(index * 2, index * 2 + 2),
  );
      ...
      <div className="mx-auto max-w-[1480px] px-4 py-7 sm:px-6 lg:px-10 lg:py-11">
            ...
            return (
              <button
                ...
                className={`flex shrink-0 items-center gap-2 rounded-full border px-4 py-2.5 text-[13px] font-semibold transition ${
                  active ? "border-[#007185] bg-[#E7F4F5] text-[#005F6B]" : "border-[#D5D9D9] bg-white text-[#0F1111] hover:border-[#AAB7B8]"
                }`}
              >
                {meta.icon}
                {meta.label}
        ...
        >
          {categoryColumns.map((column, columnIndex) => (
            <div key={`${activeHub}-column-${columnIndex}`} className="w-[calc((100%-0.75rem)/2)] shrink-0 snap-start space-y-3 sm:w-[calc((100%-1.5rem)/3)] lg:contents">
              {column.map((category) => (
                <Link
                  key={category.path}
                  href={category.path}
                ...
                >
                  <div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-[#F7F8F8] lg:aspect-square">
                    <Image loader={amazonImageLoader} src={category.imageSrc} alt="" fill sizes="(max-width: 640px) 46vw, 260px" className="object-contain p-2 transition duration-300 group-hover:scale-[1.04]" />
                  </div>
        ...
        <div className="mt-3 flex items-center justify-center gap-2 lg:hidden" role="group" aria-label="Paginação das categorias">
          {[0, 1, 2].map((page) => {
            ...
            return (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-200 ${active ? "w-6 bg-[#007185]" : "w-2 bg-[#C7CCCC] hover:bg-[#879596]"}`}
