            ...
            <div className="md:pt-1 lg:pt-2">
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:max-w-2xl">
                <HeroInfoCard
                  ...
                  title="PREÇO POR USO"
                  description="Compare por dose, unidade, peso ou rendimento."
                />
                <HeroInfoCard
                  icon={<BarChart3 className="h-4 w-4" />}
                  title="HISTÓRICO DE PREÇOS"
                  description="Veja a evolução do preço e compre no melhor momento."
                  accent="amber"
                />
                <HeroInfoCard
                  icon={<Heart className="h-4 w-4" />}
                  title="ACOMPANHE PRODUTOS"
                  description="Salve qualquer produto da Amazon e deixe o sistema acompanhar."
                  accent="emerald"
                />
                <HeroInfoCard
                  icon={<Bell className="h-4 w-4" />}
                  title="ALERTA DE QUEDA"
                  description="Se o preço cair, você é avisado."
                  accent="rose"
                />
              </div>
            </div>
...
function HeroInfoCard({
  ...
  return (
    <div className={`rounded-2xl border p-3 text-left shadow-sm ${accentStyles}`}>
      <div className="flex items-start gap-2">
        <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-white/90 shadow-sm">
          {icon}
        </span>
        <div className="min-w-0">
          <p className="text-[12px] font-semibold uppercase tracking-[0.5px] leading-[1.2] md:text-[14px]">
            {title}
          </p>
          <p className="text-[11px] leading-[1.35] text-slate-600/85 md:text-[13px]">
            {description}
          </p>
        </div>
      </div>
