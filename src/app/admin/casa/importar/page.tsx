...
import { importCasaViaAPI } from './actions';
...
export default function ImportadorCasaAPI() {
  const [asins, setAsins] = useState('');
  const [categories, setCategories] = useState<Category[]>([]); // Tipagem aplicada aqui
  const [selectedCat, setSelectedCat] = useState('');
  const [loading, setLoading] = useState(false);
  const [logs, setLogs] = useState<string[]>([]);

  useEffect(() => {
    // 2. Cast seguro do retorno da Action
  ...
  const handleImport = async () => {
    ...
    const res = await importCasaViaAPI(asins, selectedCat);
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto text-black bg-white min-h-screen font-sans">
      <div className="flex items-center justify-between mb-8 border-b pb-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Importador via API</h1>
          <p className="text-gray-500 text-sm">Os produtos entrarão no site com preço real e link de afiliado.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          <div>
            <label className="block text-xs font-black text-gray-400 uppercase tracking-tighter mb-2">1. Categoria Alvo</label>
            <select 
          ...
          <div>
            <label className="block text-xs font-black text-gray-400 uppercase tracking-tighter mb-2">2. Lista de ASINs</label>
            <textarea 
              className="w-full border border-gray-200 p-4 h-72 rounded-xl font-mono text-sm shadow-inner focus:ring-2 focus:ring-yellow-400 outline-none transition-all resize-none"
              placeholder="B0CFYRC6M7, B07XYZ..."
              value={asins}
        ...
        <div className="bg-[#131921] rounded-2xl p-6 h-[620px] overflow-auto shadow-2xl border border-gray-800">
          ...
          <div className="space-y-3 font-mono text-[10px]">
            {logs.map((log, i) => (
              <div key={i} className={`flex gap-3 leading-relaxed ${log.includes('✅') ? 'text-green-400' : 'text-red-400'}`}>
                <span className="text-gray-600 shrink-0">[{new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}]</span>
                <span className="break-all">{log}</span>
              </div>
            ))}
            {logs.length === 0 && <div className="text-gray-600 italic">Aguardando ASINs para processamento...</div>}
          </div>
        </div>
      </div>
    </div>
