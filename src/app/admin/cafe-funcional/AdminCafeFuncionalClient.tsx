...
import {
  createCafeFuncionalAction,
  deleteCafeFuncionalAction,
  updateCafeFuncionalAction,
  bulkUpdateCafeFuncionalAction,
  bulkDeleteCafeFuncionalAction,
} from "./actions";
import type { Product, FunctionalCoffeeInfo, Offer } from "@prisma/client";
import { ToastOnSubmit } from "../creatina/ToastOnSubmit"; // Ajuste o caminho se necessário

/* =======================
   TYPE DEFINITION
======================= */
export type CafeFuncionalProduct = Omit<Product, "createdAt" | "updatedAt"> & {
  functionalCoffeeInfo: FunctionalCoffeeInfo | null;
  offers: Offer[];
  createdAt: string;
  updatedAt: string;
};
...
export default function AdminCafeFuncionalClient({ products }: { products: CafeFuncionalProduct[] }) {
  ...
  const handleBulkUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const flavor = formData.get("bulkFlavor") as string; 
    ...
    await bulkUpdateCafeFuncionalAction(Array.from(selectedIds), {
      name: name || undefined,
    ...
    alert("🚀 Lote de cafés funcionais atualizado!");
  };

  const handleBulkDelete = async () => {
    if (!confirm(`Deseja excluir permanentemente ${selectedIds.size} itens?`)) return;
    await bulkDeleteCafeFuncionalAction(Array.from(selectedIds));
    setSelectedIds(new Set());
  ...
  return (
    <main className="max-w-7xl mx-auto p-6 font-sans">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-black tracking-tighter text-gray-900 uppercase">Gestão de Cafés Funcionais</h1>
      ...
      {showCreate && (
        <form action={createCafeFuncionalAction} className="space-y-4 mb-10 border border-gray-200 rounded-2xl p-6 bg-gray-50 shadow-inner">
          <ToastOnSubmit message="✅ Café funcional cadastrado!" />
          ...
          <button className="w-full bg-black text-white py-3 rounded-xl font-bold mt-4">SALVAR NOVO CAFÉ</button>
        </form>
      )}
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100 text-sm text-gray-600">
            {paginatedProducts.map((p) => {
              ...
              const info = p.functionalCoffeeInfo;
                ...
                <React.Fragment key={p.id}>
                  <tr className={`hover:bg-gray-50 transition-colors ${selectedIds.has(p.id) ? "bg-blue-50/50" : ""}`}>
                    ...
                    <td className="p-4 text-right space-x-3 text-nowrap">
                      ...
                      <form action={deleteCafeFuncionalAction} className="inline" onSubmit={(e) => !confirm("Excluir este item?") && e.preventDefault()}>
                  ...
                  {isEditing && (
                    <tr className="bg-blue-50/30">
                      <td colSpan={9} className="p-6">
                        <form action={updateCafeFuncionalAction} className="bg-white border p-6 rounded-2xl shadow-xl space-y-4">
