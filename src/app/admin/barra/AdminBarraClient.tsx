...
export default function AdminBarraClient({ products }: { products: BarraProduct[] }) {
  ...
  return (
    <main className="max-w-6xl mx-auto p-6">
      ...
      {selectedIds.size > 0 && (
        <div className="sticky top-4 z-50 mb-6 bg-black text-white p-4 rounded-xl shadow-2xl flex items-center gap-6 animate-in slide-in-from-top-4">
          <div className="flex flex-col">
            <span className="font-bold text-sm whitespace-nowrap">⚡ {selectedIds.size} selecionados</span>
            <button onClick={handleBulkDelete} className="text-[10px] text-red-400 font-black uppercase hover:text-red-300 text-left mt-1 underline">Excluir Lote</button>
          </div>
          <form onSubmit={handleBulkUpdate} className="flex-1 grid grid-cols-6 gap-2">
            <input name="bulkName" placeholder="Novo Nome" className="bg-gray-900 border border-gray-800 rounded p-2 text-xs outline-none focus:ring-1 ring-blue-500" />
            ...
            <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 rounded text-xs transition-colors">APLICAR</button>
          </form>
      ...
      {showCreate && (
        <form action={createBarraAction} className="space-y-4 mb-10 border border-gray-200 rounded-2xl p-6 bg-gray-50 shadow-inner">
          ...
          <button className="w-full bg-black text-white py-3 rounded-xl font-bold mt-4">SALVAR</button>
        </form>
      )}
                  ...
                  {isEditing && (
                    <tr className="bg-blue-50/30">
                      <td colSpan={6} className="p-6">
                        <form action={updateBarraAction} className="grid grid-cols-2 gap-6 bg-white border p-6 rounded-2xl shadow-xl">
                          <input type="hidden" name="id" value={p.id} />
                          <div className="space-y-4">
                            <Field label="Nome"><input name="name" defaultValue={p.name} className={inputStyle} /></Field>
                          ...
                          <div className="col-span-2 flex justify-end gap-3 pt-4 border-t"><button type="submit" className="bg-blue-600 text-white px-8 py-2 rounded-xl font-bold">SALVAR</button></div>
                        </form>
                      </td>
                    </tr>
                  )}
