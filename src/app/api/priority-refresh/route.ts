...
export async function POST(request: NextRequest) {
  try {
    ...
    let result: Awaited<ReturnType<typeof enqueuePriorityRefresh>> | { enqueued: false; reason: string } =
      { enqueued: false, reason: "product_not_found" };
    ...
    if (product) {
      ...
      result = priorityTouch?.shouldEnqueue && priorityTouch.asin
