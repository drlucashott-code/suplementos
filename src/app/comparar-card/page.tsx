...
type Variant = "atual" | "proposto" | "esquerda";
...
function CircleBtn({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <span
...
function MockCard({ v, p }: { v: Variant; p: Sample }) {
  const limpo = v !== "atual";
  const tabelaNaEsquerda = v === "esquerda";
  const priceColor = p.discountPercent ? "text-[#CC0C39]" : "text-[#0F1111]";

  const fotoComCoracao = (
    <>
    ...
    <div className="relative flex min-h-[230px] items-stretch gap-3 rounded-xl border border-[#D5D9D9] bg-white font-sans shadow-[0_1px_3px_rgba(15,17,17,0.06)]">
      ...
      {tabelaNaEsquerda ? (
        <div className="flex w-[150px] flex-shrink-0 flex-col gap-2 border-r border-[#EEF0F0] p-2">
          ...
          <AnaliseTabela p={p} estilo="limpo" />
        </div>
      ) : (
        <div className="relative flex w-[160px] flex-shrink-0 items-center justify-center rounded-l-xl bg-[#f3f3f3] p-2">
          {limpo ? (
            ...
            <>
              ...
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={p.image} alt={p.name} className="h-auto max-h-[190px] w-auto max-w-[130px] object-contain mix-blend-multiply" />
            </>
          )}
        </div>
      )}
      ...
      <div className="flex min-w-0 flex-1 flex-col py-3 pr-2">
        ...
        {!tabelaNaEsquerda ? (
          <div className="mb-2">
            <AnaliseTabela p={p} estilo={limpo ? "limpo" : "atual"} />
          </div>
        ) : null}
          ...
          <div className="flex items-end justify-between gap-2">
            <div className="flex items-start">
              <div className="flex items-start">
                ...
                <span className={`text-3xl font-medium leading-none tracking-tight ${priceColor}`}>{p.price[0]}</span>
                <span className={`mt-[3px] text-[14px] font-medium leading-none ${priceColor}`}>{p.price[1]}</span>
              </div>
              {!p.refPrice ? (
...
function Tag({ text, color }: { text: string; color: string }) {
  return (
    <div
      className="inline-flex rounded px-2.5 py-1 text-[12px] font-bold uppercase tracking-wide text-white"
...
export default function CompararCardPage() {
  return (
    <div className="min-h-screen bg-[#EAEDED] pb-16">
      ...
      <div className="mx-auto max-w-[430px] space-y-10 px-3 pt-4">
        {SAMPLES.map((p, i) => (
          <div key={i} className="space-y-2">
            <Tag text="Atual" color="#2162A1" />
            <MockCard v="atual" p={p} />

            <div className="pt-3">
              <Tag text="Proposto A — foto limpa" color="#067D62" />
            </div>
            <MockCard v="proposto" p={p} />

            <div className="pt-3">
              <Tag text="Proposto B — foto + tabela à esquerda" color="#7A3DB8" />
            </div>
        ...
        <div className="rounded-xl border border-[#D8DEE6] bg-white p-4 text-[13px] leading-6 text-[#475467]">
          <p className="mb-2 font-bold text-[#0F1111]">Diferenças</p>
